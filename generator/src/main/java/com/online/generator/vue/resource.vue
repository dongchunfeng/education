    ...
    methods: {
      ...
      save(formName){
        ...
        this.$refs[formName].validate(async (valid) => {
          if (valid) {
            const res = await this.$api.BUSINESS_RESOURCE_ADD(this.form);
          if(res.code==0){
            this.$notify({
            title: '成功',
            message: res.msg,
      ...
      del(id){
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          const res = await this.$api.BUSINESS_RESOURCE_DEL(id);
          if(res.code==0){
              this.$notify({
              title: '成功',
              message: res.msg,
      ...
      async getTableData () {
        try {
          const res = await this.$api.BUSINESS_RESOURCE(this.params);
          console.log("分页查询资源:");
          console.log(res);
